...
def compute_gradient_saliency_maps(samples: torch.tensor,
    ...
    # (3)
    true_scores = scores[np.arange(true_labels.shape[0]), true_labels]
    # (4)
    true_scores.backward(torch.ones_like(true_scores))
    # (5)
    gradients = samples.grad  # gradients=delta(true_scores)/delta(samples)
    # (6)
    abs_gradients = torch.abs(gradients)
    # (7)
    saliency = torch.max(abs_gradients,dim=1).values
